...
public class JalaliCalendar extends Calendar {
    ...
    JalaliCalendar(int year, int month, int dayOfMonth,
        ...
        YearMonthDate yearMonthDate = jalaliToGregorian(
                new YearMonthDate(fields[1], fields[2], fields[5]));
    ...
    public static YearMonthDate gregorianToJalali(YearMonthDate gregorian) {
        ...
        if (gregorian.getMonth() > 1 &&
                ((gregorian.getYear() % 4 == 0 && gregorian.getYear() % 100 != 0)
    ...
    public static YearMonthDate jalaliToGregorian(YearMonthDate jalali) {
        ...
        for (i = 0; gregorianDayNo >= gregorianDaysInMonth[i] + ((i == 1 && leap == 1) ? i : 0);
             i++) {
    ...
    protected void computeFields() {
        ...
        if (!areFieldsSet) {
            ...
            //Day_of_Week
            super.set(DAY_OF_WEEK, dayOfWeek(
                    jalaliToGregorian(new YearMonthDate(fields[1], fields[2], fields[5]))));
            //***
    ...
    public void add(int field, int amount) {
        ...
        if (field == MONTH) {
            ...
            YearMonthDate yearMonthDate = jalaliToGregorian(
                    new YearMonthDate(get(YEAR), get(MONTH), get(DATE)));
            Calendar gc = new GregorianCalendar(
                    yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDate(),
                    get(HOUR_OF_DAY), get(MINUTE), get(SECOND));
            gc.add(field, amount);
            yearMonthDate = gregorianToJalali(
                    new YearMonthDate(gc.get(YEAR), gc.get(MONTH), gc.get(DATE)));
            super.set(YEAR, yearMonthDate.getYear());
            super.set(MONTH, yearMonthDate.getMonth());
            super.set(DATE, yearMonthDate.getDate());
